...
class HemsServiceWorkshop(HemsServiceWorkshopBase): 
    ...
    def optimize_consumption(self, param_dict : dict, simulation_time : datetime, time_step_number : TimeStepInformation, esdl_id : EsdlId, energy_system : EnergySystem):
        ...
        residual_demand = current_active_power - pv_active_power
        if residual_demand < 0:
            # charge battery
            active_power_to_charge = np.clip(residual_demand, max=max_charge_active_power)
        else:
            # discharge battery
            active_power_to_charge = np.clip(residual_demand, min=-max_discharge_active_power)
